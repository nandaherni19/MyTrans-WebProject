...
@section('content')
    ...
    <div class="page-header">
        ...
        <div class="page-header-actions">
            ...
            <a href="{{ route('dashboard.superadmin.kelola-laporan-transaksi.export-pdf') }}" class="btn-secondary">
    ...
    <div class="dashboard-grid">
        ...
        <div class="chart-card">
            <div class="chart-header">
                <div>
                    ...
                    <p>Analitik per forma keuangan tahun {{ date('Y') }}</p>
                </div>
        ...
        <div class="right-col">
            ...
            <div class="quick-actions-card">
                ...
                <a href="{{ route('dashboard.superadmin.kelola-kendaraan') }}" class="action-btn">
                    <div class="action-icon teal"><i class="fa-solid fa-bus"></i></div>
                    <div class="action-label-wrap">
                        <span class="action-label">Tambah Kendaraan</span>
                        <span class="action-desc">Kelola armada kendaraan</span>
                    </div>
                ...
                {{-- Promo / Banner Card --}}
                <a href="{{ route('dashboard.superadmin.kelola-paket-wisata') }}" class="promo-card">
                    <div class="promo-text">
                        <div class="promo-label">🏆 Paket Terlaris</div>
                        <div class="promo-title" style="font-size: 15px; line-height: 1.4;">
                            {{ optional($paketTerlaris?->paket)->nama_paket ?? 'Belum ada data' }}
                        </div>
                        <div class="promo-sub">
                            {{ $paketTerlaris?->total_booking ?? 0 }} booking &bull;
                            Rp {{ number_format(optional($paketTerlaris?->paket)->harga ?? 0, 0, ',', '.') }}
                        </div>
                    </div>
                </a>
            </div>
    ...
    <div class="recent-card">
        <div class="recent-header">
            ...
            <a href="{{ route('booking.index') }}" class="view-all">Lihat Semua →</a>
        </div>
        <table class="booking-table">
            ...
            <tbody>
                ...
                @forelse(($recentBookings ?? []) as $i => $booking)
                    ...
                    @php
                        $bookingId = data_get($booking, 'id_booking', 0);
                        $userNama = data_get($booking, 'user.nama', 'User');
                        $namaPaket = data_get($booking, 'paket.nama_paket', '-');
                        $statusBooking = data_get($booking, 'status_booking', 'pending');
                        $totalBiaya = data_get($booking, 'total_biaya', 0);

                        $pill = match ($statusBooking) {
                            'aktif' => ['confirmed', 'Aktif'],
                            'pending' => ['pending', 'Menunggu'],
                            'batal' => ['batal', 'Batal'],
                            'selesai' => ['selesai', 'Selesai'],
                            default => ['pending', ucfirst($statusBooking)],
                        };
                    @endphp
                    ...
                    <tr>
                        <td>
                            <span class="booking-id">
                                #BK-{{ str_pad($bookingId, 3, '0', STR_PAD_LEFT) }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="customer-info">
                                <div class="avatar {{ $avatarColors[$i % 5] }}">
                                    {{ strtoupper(substr($userNama, 0, 2)) }}
                                </div>
                        ...
                        <td>
                            <a href="{{ route('booking.show', 'BK' . str_pad($bookingId, 3, '0', STR_PAD_LEFT)) }}"
