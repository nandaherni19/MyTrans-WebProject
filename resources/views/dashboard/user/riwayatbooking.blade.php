            ...
            <div class="booking-list">
                @forelse($riwayat as $item)
                    ...
                    <div class="booking-card"
                        data-search="{{ strtolower(($item['judul'] ?? '').' '.($item['booking_id'] ?? '').' '.($item['lokasi'] ?? '')) }}"
                        data-date="{{ !empty($item['tanggal']) ? \Carbon\Carbon::parse($item['tanggal'])->format('Y-m-d') : '' }}"
                        >
            ...
            <section class="detail-card">
                <div class="detail-top">
                    <div class="detail-image">
                        <img src="{{ !empty($detail['gambar']) ? asset($detail['gambar']) : asset('img/jatimPark2.jpg') }}"
                            alt="Detail Booking">
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // kalau search aktif → abaikan filter tanggal
    const selectedDate =
        keyword !== ''
        ? ''
        : dateInput.value;
    ...
    // otomatis tampil booking hari ini
    const today = new Date().toISOString().split('T')[0];
    dateInput.value = '';

    filterBooking();

    searchInput.addEventListener('input', filterBooking);
    dateInput.addEventListener('change', filterBooking);
