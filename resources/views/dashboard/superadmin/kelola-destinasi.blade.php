...
@section('content')
    ...
    <div class="tabs">
        <a href="{{ url('/dashboard/superadmin/kelola-destinasi/provinsi') }}"
            class="{{ $isProvinsi ? 'active' : '' }}">Provinsi</a>
        <span>|</span>
    ...
    <section class="card-box">
        @if($isProvinsi)
            ...
            {{-- FORM TAMBAH --}}
            <form id="formTambahProvinsi" action="{{ route('dashboard.superadmin.kelola-destinasi.provinsi.store') }}"
                method="POST" class="provinsi-add-wrap" style="display:none;">
                @csrf
            ...
            {{-- FORM EDIT --}}
            <form id="formEditProvinsiInline" method="POST" class="provinsi-add-wrap" style="display:none;">
                @csrf
                @method('PUT')
                <div class="provinsi-add-row">
                ...
                @forelse($provinsis as $provinsi)
                    <div class="item-card">
                        ...
                        <div class="item-actions">
                            <button type="button" class="icon-edit btn-edit-provinsi" data-id="{{ $provinsi->id_provinsi }}"
                                data-nama="{{ $provinsi->nama_provinsi }}">
                                <i class="fa-solid fa-pen"></i>
                            </button>

                            <button type="button" class="icon-delete btn-delete-provinsi" data-id="{{ $provinsi->id_provinsi }}">
        ...
        @if($isKota)
            ...
            {{-- FORM TAMBAH KOTA --}}
            <form id="formTambahKota" action="{{ route('dashboard.superadmin.kelola-destinasi.kota.store') }}" method="POST"
                class="form-box" style="display:none;">
                @csrf
            ...
            {{-- FORM EDIT KOTA --}}
            <form id="formEditKotaInline" method="POST" class="form-box" style="display:none;">
                @csrf
                @method('PUT')
                <div class="form-grid two-col">
            ...
            <div class="item-list">
                @forelse($kotas as $kota)
                    <div class="item-card">
                        ...
                        <div class="item-actions">
                            <button type="button" class="icon-edit btn-edit-kota" data-id="{{ $kota->id_kota }}"
                                data-nama="{{ $kota->nama_kota }}" data-provinsi="{{ $kota->id_provinsi }}">
                                <i class="fa-solid fa-pen"></i>
                            </button>

                            <button type="button" class="icon-delete btn-delete-kota" data-id="{{ $kota->id_kota }}">
                                <i class="fa-solid fa-trash"></i>
                            </button>
                        </div>
                    </div>
    ...
    <div id="modalDeleteGeneric"
        ...
        <div
            style="background:#fff; border-radius:14px; padding:2rem 1.75rem; max-width:380px; width:90%; text-align:center; font-family:'Poppins',sans-serif;">
            <div
                style="width:52px; height:52px; border-radius:50%; background:#fef2f2; display:flex; align-items:center; justify-content:center; margin:0 auto 1rem;">
