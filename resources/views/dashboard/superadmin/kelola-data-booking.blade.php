            ...
            <div class="booking-filter">
                <div class="search-box">
                    ...
                    <input
                        type="text"
                        id="searchBooking"
                        placeholder="Cari ID booking, nama, paket, kendaraan..."
                ...
                <div class="form-container">
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-grid">
                            ...
                            @if($current['tipe_booking'] === 'paket')
                                <div class="form-group">
                                    ...
                                    <input type="date" name="tanggal_berangkat" class="form-select" value="{{ $current['tanggal_berangkat_raw'] }}">
                                </div>

                                <div class="form-group">
                                    <label><i class="fa-regular fa-calendar"></i> Tanggal Kembali</label>
                                    <input type="date" name="tanggal_kembali" class="form-select" value="{{ $current['tanggal_kembali_raw'] }}">
                                </div>
                            @else
                                <div class="form-group">
                                    <label><i class="fa-regular fa-calendar"></i> Tanggal Pemberangkatan</label>
                            ...
                            @if(($current['tipe_booking'] ?? '') === 'open_trip')
                                <div class="form-group">
                                    <label><i class="fa-solid fa-location-dot"></i> Titik Jemput</label>
                                    <div class="input-box">{{ $current['titik_jemput'] ?? '-' }}</div>
                                </div>
                            @else
                                <div class="form-group">
                        ...
                        <div class="form-card">
                            ...
                            @else
                                <p style="color:#6b7280; margin-bottom:10px;">
                                    Pilih kendaraan tambahan. Setiap kendaraan hanya bisa digunakan untuk satu booking pada tanggal yang sama.
                                </p>
                                ...
                                <div id="selectedKendaraanList">
                                    @foreach($current['kendaraan_list'] as $k)
                                        <div class="selected-kendaraan-item" id="kendaraan-item-{{ $k->id_kendaraan }}">
                                            ...
                                            <button type="button" onclick="hapusKendaraan('{{ $k->id_kendaraan }}')" class="btn-remove-kendaraan">
                                                Hapus
                                            </button>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="status_booking" class="form-select">
                                    <option value="pending" {{ $current['status_booking']=='pending'?'selected':'' }}>Pending</option>
                    ...
                    <div class="form-action">
                        <a href="{{ url()->previous() != url()->current() ? url()->previous() : route('dashboard.superadmin.kelola-data-booking') }}" class="btn-cancel">Kembali</a>                        <button type="submit" class="btn-save">Ubah</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    @if(($page ?? '') === 'detail' && $current)
        ...
        <div class="main-scroll">
            <div class="form-container">
                ...
                <div class="form-action">
                    <a href="{{ url()->previous() != url()->current() ? url()->previous() : route('dashboard.superadmin.kelola-data-booking') }}" class="btn-cancel">Kembali</a>
                    {{-- BATALKAN --}}
            ...
            <!-- Modal Batal Booking -->
            <div id="modalBatal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:999; align-items:center; justify-content:center;">
            ...
            <div id="modalLunasi" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:999; align-items:center; justify-content:center;">
                <div style="background:#fff; border-radius:16px; padding:32px; max-width:420px; width:90%;">
            ...
            {{-- Modal QR Pelunasan --}}
            <div id="modalQrisPelunasan" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:1000; align-items:center; justify-content:center;">
        ...
        <div class="form-container" style="max-width:600px; width:90%;">
            ...
            <div id="stepForm" class="form-card">
                ...
                <div class="form-grid">
                        ...
                        <select name="id_paket" id="adminPaket" class="form-select" required>
                            ...
                            @foreach($pakets as $paket)
                                ...
                                <option
                                    ...
                                    data-tipe="{{ $paket->tipe }}"
                                    data-harga="{{ $paket->harga }}"
                                    data-nama="{{ $paket->nama_paket }}"
                                    data-min="{{ $paket->min_peserta ?? 1 }}"
                                    data-sisa="{{ $paket->sisa_kursi }}"
                                    {{ $habis ? 'disabled' : '' }}
                                >
                                    {{ $paket->nama_paket }}
                    ...
                    <div class="form-group" id="wrapTitikJemput" style="display:none;">
                        <label>Titik Jemput <span>*</span></label>
                        <select id="modalTitikJemput" class="form-select">
            ...
            <form id="formBookingSubmit" method="POST"
                action="{{ route('dashboard.superadmin.kelola-data-booking.store') }}"
                style="display:none;">
                @csrf
                ...
                <input type="hidden" name="id_titik_jemput" id="fTitikJemput">
    ...
    $paketData = $pakets->mapWithKeys(function ($paket) {
        return [
            $paket->id_paket => [
                ...
                'titik_jemput' => $paket->titikJemput->map(function ($titik) {
                    return [
                        'id_titik_jemput' => $titik->id_titik_jemput,
                        'nama' => $titik->nama,
                    ];
                })->values(),
            ],
...
function fetchKendaraanTersedia() {
    ...
    fetch(`/dashboard/superadmin/kelola-data-booking/kendaraan-tersedia?tanggal_berangkat=${tglBerangkat}&tanggal_kembali=${tglKembali}`)
        .then(r => r.json())
        .then(data => {
            semuaKendaraan = data;
...
function submitBooking() {
    ...
    document.getElementById('fTitikJemput').value = document.getElementById('modalTitikJemput').value;
...
function bukaModalBatal(idBooking) {
    ...
    fetch(`/dashboard/superadmin/kelola-data-booking/sisa/${idBooking}`)
        ...
        .then(data => {
            ...
            const refund = Math.floor(sudahBayar * 0.85);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const titikSelect = document.getElementById('modalTitikJemput');
    const wrapTitik = document.getElementById('wrapTitikJemput');
    ...
    function updateAreaJemput() {
        ...
        titikSelect.innerHTML = '<option value="">-- Pilih Titik Jemput --</option>';

        wrapTitik.style.display = 'none';
        ...
        if (data.tipe === 'open_trip') {
            wrapTitik.style.display = '';
            titikSelect.required = true;

            data.titik_jemput.forEach(titik => {
                titikSelect.innerHTML += `<option value="${titik.id_titik_jemput}">${titik.nama}</option>`;
            });
        } else {
