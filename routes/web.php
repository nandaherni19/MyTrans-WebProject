...
use App\Http\Controllers\User\RequestWisataUserController;
...
Route::patch('/dashboard/superadmin/kelola-data-booking/{id}/refund-selesai',
...
Route::prefix('/dashboard/superadmin/kelola-laporan-transaksi')->controller(LaporanTransaksiController::class)->middleware(['auth', 'role:admin,superadmin'])->group(function () {
    ...
    Route::get('/export-csv', 'exportCsv')->name('dashboard.superadmin.kelola-laporan-transaksi.export');
...
Route::get('/dashboard/user/riwayatbooking/{filter?}/{page?}', 
    [RiwayatBookingUserController::class, 'index']
)->middleware(['auth', 'role:user'])->name('dashboard.user.riwayatbooking');

// DETAIL PESANAN - USER
Route::get('/dashboard/user/detail-pesanan/{jenis}/{id}', function ($jenis, $id) {
    if ($jenis === 'booking') {
        $data = Booking::with(['paket.kota.provinsi', 'pembayaranTerakhir'])->findOrFail($id);
    } else {
        $data = RequestWisata::with(['kotaAsal', 'kotaTujuan'])->findOrFail($id);
    }
    return view('dashboard.user.detailpesanan', compact('data', 'jenis'));
...
// BOOKING - dari Request Wisata (disetujui)
Route::get('/dashboard/user/booking/request/{id_request}', [BookingController::class, 'bookingRequest'])
    ->middleware(['auth', 'role:user'])
    ->name('dashboard.user.booking');
